...
erDiagram
    ...
    BOOKINGS {
        ...
        uuid user_id FK "NOT NULL"
        uuid order_id FK "nullable в схеме, задан у любой брони"
...
`order_id` заполняется сразу при создании брони и на практике не бывает пустым.
Заказ и брони создаются одной транзакцией: `POST /orders` порождает заказ в
статусе `PENDING` и по строке в `bookings` со статусом `HELD` на каждое выбранное
место. Жизненный цикл выражается переходами статусов: `HELD + PENDING` →
`PAID + PAID` при успешной оплате, либо `EXPIRED + CANCELLED` по истечении
`expires_at`.

Ранняя редакция документа предполагала `HELD` без заказа с последующим созданием
`PENDING`-заказа в момент оплаты. От этого отказались: удержание без владельца —
состояние, которое некому отменить и не на чем показать таймер, а клиенту
пришлось бы вести две сущности вместо одной. Столбец оставлен nullable ради
самого порядка вставки внутри транзакции и совместимости с ручными правками
данных, но инвариант приложения — `order_id` задан у любой брони.
