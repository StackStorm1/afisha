...
class Session(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            'base_price >= 0', name='ck_sessions_base_price'
        ),
        CheckConstraint(
            'seats_total > 0', name='ck_sessions_seats_total'
        ),
        CheckConstraint(
            'seats_available BETWEEN 0 and seats_total', name='ck_sessions_seats_available'
        ),
        CheckConstraint(
    ...
    event_id: Mapped[UUID] = mapped_column(ForeignKey('events.id', name='fk_sessions_event', ondelete='RESTRICT'))
    venue_id: Mapped[UUID] = mapped_column(ForeignKey('venues.id', name='fk_sessions_venue', ondelete='RESTRICT'))
