...
class CreateOrderRequest(BaseModel):
    session_id: UUID
    seat_ids: list[UUID] = Field(..., description='UUID мест, выбранных на схеме зала', min_length=1, max_length=10)
    
class BookedSeat(BaseModel):
    model_config = ConfigDict(
        json_schema_extra={
            "description": "Место в составе заказа"
        }
    )
    ...
    price_category: PriceCategory = Field(..., description='Ценовая категория места. В БД — в верхнем регистре.', examples=['stalls'])
    price: Money
    
class OrderEventRef(BaseModel):
    id: UUID
    title: str = Field(..., examples=['Гамлет'])
    poster_url: str | None = Field(None, examples=['https://cdn.example.com/hamlet.jpg'])
...
class OrderSessionRef(BaseModel):
    model_config = ConfigDict(
        json_schema_extra={
            "description": "Краткая информация о сеансе внутри брони"
        }
    )
    id: UUID
    event: OrderEventRef
...
class OrderDetail(BaseModel):
    ...
    expires_at: AwareDatetime | None = Field(None, description='Время истечения удержания мест. Заполнено только для статуса pending (BR-02).', examples=['2026-10-01T10:15:00Z'])
